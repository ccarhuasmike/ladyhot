    ...
    public class ApiSeguridadController : ApiController
    {
        ...
        public ClientResponse EnvioEmailGenerarContrasnia(Bean_mail beanMail)
        {
            ...
            try
            {
                ...
                IEnumerable<Tbl_parameter_detDto> lista_site_lady_hot = new ParameterLogic().GetParameter_skey(entidad_det_site_lady_hot);
                Tbl_parameter_detDto site_lady_hot = lista_site_lady_hot.ToList().Where(x => x.skey_det.Equals("SKEY_SITE_LADYHOT_RUTA")).FirstOrDefault();
        ...
        [Route("ReestablecerContrasnia"), HttpPost]
        public ClientResponse ReestablecerContrasnia(Bean_mail beanMail)
        {
            ClientResponse clientResponse = new ClientResponse();
            try
            {
                ...
                var url = System.Web.Hosting.HostingEnvironment.MapPath("/template/templatereestablecerpassword.html");
                string mensaje = "";
                mensaje = Utilidades.LeerTemplateHTML(url);
                Tbl_usuario entididad_usuario = new UsuarioLogic().getUsuarioPorCorreo(new Tbl_usuario() { tx_email = beanMail.para.First() });
                mensaje = mensaje.Replace("{email}", beanMail.para.First());
